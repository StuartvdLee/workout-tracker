...
function initForm(): void {
  ...
  initSortableList({
    listId: "workout-selected-list",
    announceId: "workout-reorder-announce",
    getArray: () => selectedExercises,
    onReorder: renderExerciseDropdown,
  });
}
...
async function fetchAndPopulateEditModal(workoutId: string, nameInput: HTMLInputElement, backdrop: HTMLElement): Promise<void> {
  try {
    ...
    initSortableList({
      listId: "edit-selected-list",
      announceId: "edit-reorder-announce",
      getArray: () => editSelectedExercises,
      onReorder: renderEditExerciseDropdown,
    });
