...
let editSelectedExercises = new Set();
...
function initEditModal() {
    ...
    const cancelBtn = document.getElementById("workout-edit-cancel");
    const backdrop = document.getElementById("workout-edit-backdrop");
    if (!form || !backdrop)
        return;
    form.addEventListener("submit", (event) => {
        event.preventDefault();
    ...
    backdrop.addEventListener("keydown", (event) => {
        ...
        // Focus trap
        if (event.key === "Tab") {
            const modal = backdrop.querySelector(".edit-modal");
            if (!modal)
                return;
...
function initDeleteModal() {
    ...
    backdropEl.addEventListener("keydown", (event) => {
        ...
        // Focus trapping
        if (event.key === "Tab") {
            const focusable = backdropEl.querySelectorAll('button:not([disabled]), [tabindex]:not([tabindex="-1"])');
            if (focusable.length === 0)
                return;
            const first = focusable[0];
            const last = focusable[focusable.length - 1];
...
function renderExerciseToggles() {
...
function renderEditExerciseToggles() {
...
async function handleSubmit() {
    ...
    const apiErrorEl = document.getElementById("workout-api-error");
    const submitBtn = document.querySelector("#workout-form .workout-form__submit");
    if (!nameInput || !errorEl || !apiErrorEl || !submitBtn)
        return;
...
function openEditModal(workout) {
    ...
    const errorEl = document.getElementById("edit-workout-error");
    const apiErrorEl = document.getElementById("edit-workout-api-error");
    if (!backdrop || !nameInput)
        return;
...
async function fetchAndPopulateEditModal(workoutId, nameInput, backdrop) {
    try {
...
function renderEditExerciseDropdown() {
    const select = document.getElementById("edit-exercise-select");
    if (!select)
        return;
...
function renderEditSelectedExercisesList() {
...
function buildSelectedExerciseItem(name, onRemove) {
    const li = document.createElement("li");
...
async function handleEditSubmit() {
    if (isEditSubmitting || editingWorkoutId === null)
        return;
    const nameInput = document.getElementById("edit-workout-name");
    const errorEl = document.getElementById("edit-workout-error");
    const apiErrorEl = document.getElementById("edit-workout-api-error");
    const submitBtn = document.querySelector("#workout-edit-form .workout-form__submit");
    if (!nameInput || !errorEl || !apiErrorEl || !submitBtn)
        return;
    clearValidationError(nameInput, errorEl);
    apiErrorEl.textContent = "";
...
async function handleDelete() {
    if (isDeleting || deletingWorkoutId === null)
        return;
    const confirmBtn = document.getElementById("workout-delete-confirm");
    const errorEl = document.getElementById("workout-delete-error");
    isDeleting = true;
