...
**Goal**: When an effort value is already set from a previous render (e.g., the exercise list is rebuilt mid-session after exercise reorder, with existing effort values in `previousLogEntries`), the correct colour is applied immediately without requiring user interaction.

**Independent Test**: Set an effort value on a per-exercise slider, then trigger a re-render of the exercise list (e.g., exercise reorder). Confirm the slider and its band/value text render with the correct palette colour without needing to drag the slider again.

- [x] T011 [US3] In `renderExerciseInputs()` in `src/WorkoutTracker.Web/wwwroot/ts/pages/active-session.ts`: the function snapshots `logEntries` into `previousLogEntries` before rebuilding, then seeds each new `logEntries` entry from it. After creating the slider with default values, add a conditional block — if `existingEntry.loggedEffort !== null` (where `existingEntry` comes from `previousLogEntries`), restore `effortSlider.value`, `data-touched`, `aria-valuenow`, `aria-valuetext`, `effortValueEl.textContent`, `effortBandEl.textContent`, and apply `accentColor` + text `color` from `getEffortColour(existingEntry.loggedEffort)` — matching the full state that a user interaction would have set.
