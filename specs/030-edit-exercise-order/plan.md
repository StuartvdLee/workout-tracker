...
- **User Experience Consistency**: The order editor MUST match the established "Edit Workout" reorder pattern: six-dot drag handle, live DOM movement during drag, touch clone behavior, keyboard pick-up/move/drop, `.sr-only` announcements, and drag visual states. The top-right action belongs in `.active-session__header`; label changes to a clear exit action such as "Done" while editing. Collapsed rows show names only; weight, effort, previous data, and targets are hidden while editing.
