...
Replace the placeholder Exercises page with a fully functional exercise management interface. Users can create, view, edit, and delete exercises with a required name and optional targeted muscle associations. The backend provides CRUD API endpoints, a Muscle entity with seed data, and an ExerciseMuscle junction table. The frontend replaces the exercises placeholder with a creation form, a scrollable exercise list with pencil (edit) and trash (delete) icon buttons, a modal dialog for editing, and a confirmation dialog for deletion. All data persists in PostgreSQL via Entity Framework Core. The existing predefined muscle list (11 muscles) is stored as seeded database rows for referential integrity and API-driven display.
...
**Scale/Scope**: Single-user personal tracker; 1 page reworked, 5 new API endpoints (GET muscles, GET/POST/PUT/DELETE exercises), 1 new EF migration, ~100 E2E test cases
...
- **Testing**: Playwright E2E tests cover all five user stories: create exercise with name, assign muscles, view exercise list, edit exercise via modal, and delete exercise with confirmation. Tests cover validation (empty name, whitespace-only, max length, duplicate names), modal interactions (open, close, Escape, backdrop), delete confirmation, empty state, ARIA attributes, mobile responsiveness, and performance. The existing `WebAppFixture` mock API pattern is extended with exercise and muscle CRUD endpoints. ✅ Tests are mandatory, not optional.
...
- **User Experience Consistency**: The exercise form follows the same patterns as the Home page workout form: BEM CSS classes, inline error messages with `role="alert"` and `aria-live="polite"`, `novalidate` with custom validation, existing design tokens for spacing, colours, and typography. The exercise list uses the same `--max-content-width` constraint. Edit uses a modal dialog with ARIA (role="dialog", aria-modal="true", focus trapping). Delete uses a confirmation dialog (role="alertdialog"). Empty state follows the placeholder pattern. Touch targets meet `--min-touch-target` (44px). Muscle toggles use `@media (hover: none)` to prevent sticky hover on touch devices. ✅ All existing patterns followed.
