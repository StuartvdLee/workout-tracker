...
- [ ] T001 Confirm feature branch `007-dark-mode-toggle` is checked out and `git status` is clean
- [ ] T002 [P] Create empty `src/WorkoutTracker.Web/wwwroot/ts/theme.ts` with export placeholder so TypeScript compilation continues to pass
- [ ] T003 [P] Create empty `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` with a single passing smoke test
...
- [ ] T004 Add `[data-theme="dark"]` override block immediately after the `:root` block in `src/WorkoutTracker.Web/wwwroot/css/styles.css` with the 8 colour overrides:
...
- [ ] T005 Make the `.topbar` always visible in `src/WorkoutTracker.Web/wwwroot/css/styles.css`:
  - Change `.topbar { display: none }` to `display: flex`
  - Remove the `@media (max-width: 767px)` rule that sets `.topbar { display: flex }` (now redundant)
  - Add `@media (min-width: 768px) { .topbar { left: var(--sidebar-width); width: calc(100% - var(--sidebar-width)); } }` so the topbar appears to the right of the sidebar on desktop
  - Move `.content { padding-top: calc(var(--topbar-height) + var(--spacing-lg)) }` out of the mobile-only media query so it applies at all breakpoints

- [ ] T006 [P] Add flash-prevention inline `<script>` as the last element inside `<head>` (before `</head>`) in `src/WorkoutTracker.Web/wwwroot/index.html`:
  ```html
  <script>
    (function () {
...
- [ ] T007 [US1] Add the theme selector button and dropdown menu markup inside `.topbar` in `src/WorkoutTracker.Web/wwwroot/index.html`, following the exact structure in `specs/007-dark-mode-toggle/contracts/ui-contract.md`:
  - A wrapping `<div class="topbar__theme">` after `.topbar__title`
  - `<button id="theme-btn" class="topbar__theme-btn" type="button" aria-label="Open theme menu" aria-haspopup="menu" aria-expanded="false" aria-controls="theme-menu">`
...
- [ ] T008 [P] [US1] Add `.topbar__theme-btn` styles to `src/WorkoutTracker.Web/wwwroot/css/styles.css`:
...
- [ ] T009 [P] [US1] Add icon visibility rules and system indicator dot styles to `src/WorkoutTracker.Web/wwwroot/css/styles.css`:
...
- [ ] T010 [P] [US1] Add `.theme-menu` and `.theme-menu__item` styles to `src/WorkoutTracker.Web/wwwroot/css/styles.css`:
...
- [ ] T011 [US1] Implement `theme.ts` at `src/WorkoutTracker.Web/wwwroot/ts/theme.ts` with the full module as specified in `specs/007-dark-mode-toggle/plan.md` Workstream C:
...
- [ ] T012 [US1] Wire `initTheme()` into `src/WorkoutTracker.Web/wwwroot/ts/main.ts`:
...
- [ ] T013 [US1] Write Vitest unit tests in `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` covering `getStoredPreference()`:
...
- [ ] T014 [P] [US1] Write Vitest unit tests in `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` covering `resolveTheme()`:
  - `'light'` → `'light'`
  - `'dark'` → `'dark'`
  - `'system'` with OS dark → `'dark'`
  - `'system'` with OS light → `'light'`

- [ ] T015 [P] [US1] Write Vitest unit tests in `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` covering `applyTheme()`:
...
- [ ] T016 [US1] Run `cd src/WorkoutTracker.Web && npm test` and confirm all tests pass (including pre-existing `router.test.ts`)
...
- [ ] T017 [US2] Verify that `applyTheme()` in `src/WorkoutTracker.Web/wwwroot/ts/theme.ts` calls `localStorage.setItem('workout-tracker-theme', pref)` — confirm this write is triggered from the `.theme-menu__item` click handler in `initTheme()` (not from `applyTheme()` itself, which is also called by the FOUC script path)

- [ ] T018 [US2] Write Vitest unit tests in `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` covering persistence:
...
- [ ] T019 [US2] Run `cd src/WorkoutTracker.Web && npm test` and confirm all tests pass
...
- [ ] T020 [US3] Complete the `matchMedia 'change'` listener in `initTheme()` in `src/WorkoutTracker.Web/wwwroot/ts/theme.ts`:
...
- [ ] T021 [US3] Write Vitest unit tests in `src/WorkoutTracker.Web/wwwroot/ts/__tests__/theme.test.ts` covering real-time OS tracking:
...
- [ ] T022 [US3] Run `cd src/WorkoutTracker.Web && npm test` and confirm all tests pass
...
- [ ] T023 Verify full keyboard interaction in a browser:
...
- [ ] T024 [P] Verify WCAG AA contrast in both themes using DevTools or a browser contrast checker:
...
- [ ] T025 [P] Visual smoke test: navigate to every page in the app (home, workout list, active session, exercise detail) in both light and dark themes and confirm no unstyled elements, no invisible text, no clipped layout

- [ ] T026 [P] Run the full backend test suite `dotnet test src/WorkoutTracker.slnx` to confirm no regressions (API and integration tests are unaffected but must still pass)

- [ ] T027 Run `cd src/WorkoutTracker.Web && npm run build` (TypeScript compilation) and confirm zero errors

- [ ] T028 Commit all changes with message: `feat: add dark mode toggle with Light/Dark/System menu (#007)`
